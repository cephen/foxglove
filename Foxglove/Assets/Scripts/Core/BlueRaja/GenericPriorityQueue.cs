...
namespace Foxglove.Core.BlueRaja {
    ...
    public sealed class GenericPriorityQueue<TItem, TPriority> : IFixedSizePriorityQueue<TItem, TPriority>
        ...
        /// If node is or has been previously added to another queue, the result is undefined unless oldQueue.ResetNode(node) has
        /// been called
        /// O(1)
        /// </summary>
#if NET_VERSION_4_5
        ...
        private void OnNodeUpdated(TItem node) {
            ...
            if (parentIndex > 0
                && HasHigherPriority(node, _nodes[parentIndex]))
                CascadeUp(node);
            else
